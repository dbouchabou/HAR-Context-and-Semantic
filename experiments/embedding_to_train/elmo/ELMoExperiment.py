...
class ELMoExperiment(Experiment):
    ...
    def prepare_data_for_elmo_full_sentence(self):
        ...
        self.elmo_data_train = [" ".join(sentences)]

        #if self.DEBUG:
        print("")
        print(len(self.elmo_data_train))
        print(len(self.elmo_data_train[0]))
    ...
    def prepare_dataset(self):
        ...
        with tqdm(total=2, desc='Prepare Dataset') as pbar:
            ...
            #self.prepare_data_for_elmo_full_sentence()
            pbar.update(1)
    ...
    def start(self):
        ...
        csv_name = self.experiment_parameters["model_type"] + "_" + self.experiment_tag + ".csv"
        ...
        run_id = self.experiment_parameters["model_type"] + "_" + self.experiment_tag + "_" + str(current_time)
        log_dir = os.path.join(root_logdir, run_id)

        self.elmo_model.train(best_model_path = final_model_path, log_dir = log_dir, csv_path = csv_path)
