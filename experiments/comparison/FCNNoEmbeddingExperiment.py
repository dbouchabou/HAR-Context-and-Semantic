...
class FCNNoEmbeddingExperiment(Experiment):
    ...
    def pad_fcn(self,sequences, maxlen):
        ...
        for seq in sequences:
            if len(seq) > maxlen:
                ...
                new_seq = np.pad(seq, (0, new_len), 'symmetric')
    ...
    def prepare_data_for_classifier(self):
        ...
        #self.classifier_data_X = self.pad_fcn(self.classifier_segmentator.X, maxlen = self.experiment_parameters["sequence_lenght"])
        #print(self.classifier_data_X.shape)
        ...
        #print(self.classifier_data_X.shape)
        #print(self.classifier_data_X)
        #input("Press Enter to continue...")
        #self.classifier_data_X = tf.convert_to_tensor(self.classifier_segmentator.X, dtype=tf.int64)
    ...
    def build_model_classifier(self, run_number=0):
        ...
        feature_1 = Input(shape=((nb_timesteps,1)))

        #embedding = embedding_layer (feature_1)
        #embedding = BatchNormalization()(embedding)

        conv1 = Conv1D(filters=128, kernel_size=9, padding='same')(feature_1)
        ...
        self.classifier_model = Model(inputs=feature_1, outputs=output_layer, name="FCN_Embedding")
