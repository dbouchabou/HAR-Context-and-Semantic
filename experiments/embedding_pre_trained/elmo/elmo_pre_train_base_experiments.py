...
class ELMoPreTrainBaseExperiment(Experiment):
    ...
    def encode_dataset_for_classifier(self):
        ...
        #self.classifier_dataset_encoder.basic_raw_encoded(custom_dict = self.elmo_model.vocabulary)
    ...
    def segment_dataset_for_classifier(self):
        ...
        #self.classifier_segmentator.slidingSensorEventWindows(self.experiment_parameters["sequence_lenght"],1)
    ...
    def prepare_data_for_classifier(self):
        ...
        self.classifier_data_X = pad_sequences(encoded_sentences, maxlen = self.experiment_parameters["sequence_lenght"], padding = 'post')

        #self.classifier_data_Y, self.classifier_dataset_encoder.actDict = self.rename_labels(cairo_dict)
    ...
    def model_selection(self):
        ...
        self.classifier_data_X_train = np.array(self.classifier_data_X_train, dtype=object)
        self.classifier_data_Y_train = np.array(self.classifier_data_Y_train, dtype=object)

        self.classifier_data_X_test = np.array(self.classifier_data_X_test, dtype=object)
        self.classifier_data_Y_test = np.array(self.classifier_data_Y_test, dtype=object)
        bar.next()
    ...
    def train(self, X_train_input, Y_train_input, X_val_input, Y_val_input, run_number=0):
        ...
        #X_train, X_validation, y_train, y_validation = train_test_split(X_train_input, Y_train_input, test_size=0.2, random_state=7, shuffle = True, stratify = Y_train_input)
       ...
       #self.classifier_model.fit(X_train, 
       #               y_train, 
       #               epochs = self.experiment_parameters["nb_epochs"],
       #               batch_size=self.experiment_parameters["batch_size"], 
       #               verbose=self.experiment_parameters["verbose"],
